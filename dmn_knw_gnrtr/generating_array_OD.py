...
def process_data(top_D_stations, base_dir, prefix, str_prdc_attr, seq_len):
    os.makedirs(base_dir, exist_ok=True)
    result_array_file_path = os.path.join(base_dir, f'{prefix}_result_array.pkl')
    ...
    for i in range(0, T - seq_len - seq_len + 1):
        ...
        if (pd.to_datetime('05:15:00').time() > lastest_time.time() >= pd.to_datetime('00:13:00').time() or
                pd.to_datetime('00:13:00').time() < earliest_time.time() < pd.to_datetime('05:15:00').time()):
            continue
...
def process_data_D(top_D_stations, base_dir, prefix, str_prdc_attr, seq_len):
    os.makedirs(base_dir, exist_ok=True)
    result_array_file_path = os.path.join(base_dir, f'{prefix}_result_array.pkl')
    train_dict_file_path = os.path.join(base_dir, f'{prefix}.pkl')
